...
/* 
char spi(unsigned char byte) {
	SSPBUF = byte;
	while(!(SSPSTATbits.BF));
	return SSPBUF;
}
 
unsigned char ATA_command(unsigned char id1, unsigned char id2,unsigned char command,unsigned char data) {
	unsigned char rdata;
	delay(20);
	CHIP_SELECT = 0;
	spi(id1);
...
unsigned char DetectNummberOfATAchips() {
    ...
    for (chip = 0; chip < NUMBER_OF_ATA_CHIPS; chip++) { //genereramo 2 bajta za naslov ata ?ipa
       ...
       // if (ATA_command(Id1, Id0, 0x0C,0x00)!= 0)break;
    }
    return chip;
}
...
//uart can emulacija
            unsigned char a,b;
            for(a=0;a<noOfATAChips;a++){
                putsUSART(BatteryVoltage[a]);
                putcUSART(',');
            }
